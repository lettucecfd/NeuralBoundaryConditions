...
class AverageDensityField(lt.Reporter):
    """General VTK Reporter for velocity and pressure"""

    def __init__(self, interval=50, start_i_1=0, start_i_2=0, config = None):
        super().__init__(interval)
        self.rho_average = None
        self.rho_fluct_average = None
        self.rho_rms = None
        self.config = config
        self.id_1 = 1.0
        self.id_2 = 1.0
        self.start_i_1 = start_i_1
        self.start_i_2 = start_i_2

        # Liste zum Speichern des Fehlers über die Zeit
        self.convergence_history = []

    def __call__(self, simulation: 'Simulation'):
        self.convergence_history.append(simulation.flow.rho().max())
        if simulation.flow.i > self.start_i_2:
            # Sicherheitshalber prüfen, ob Phase 1 erfolgreich war
            if self.rho_average is None:
                return

            if self.rho_fluct_average is not None:
                scaler = (self.id_2)/(self.id_2+1)
                self.rho_fluct_average = self.rho_fluct_average*scaler + (simulation.flow.rho()-self.rho_average)**2*(1-scaler)
                self.id_2 += 1
            else:
                self.rho_fluct_average = (simulation.flow.rho()-self.rho_average)**2
            return
        if simulation.flow.i > self.start_i_1:
            if self.rho_average is not None:
                scaler = (self.id_1)/(self.id_1+1)
                self.rho_average = self.rho_average*scaler + simulation.flow.rho()*(1-scaler)
                self.id_1 += 1
            else:
                self.rho_average = simulation.flow.rho()
                self.convergence_history.append(1.0)
            return
    ...
    def postprocess(self):
        ...
        aoa = self.config["aoa"]
        re = self.config["Re"]
        k_str = self.config["k_str"]
        torch.save(torch.tensor(self.convergence_history).cpu(), f"crash_gpd{gpd}_re{re}_aoa{aoa}_k{k_str}_v3.pt")
        if self.rho_fluct_average is None or self.rho_average is None:
            return 0.0  # Oder eine Warnung ausgeben
        return (torch.sqrt(self.rho_fluct_average) / self.rho_average).cpu()
...
def run(context, config, K, dataset, dataset_nr, t_lu):
    ...
    flow = NACA0012(context, [config["nx"]+config["extension"], config["ny"]],
                    reynolds_number=config["Re"],
                    mach_number=config["Ma"],
                    velocity_init=1,
                    K=K,
                    xc=config["xc"],
                    distanceFromRight=200+config["extension"],
                    gpd=config["gpd"], cx=250, cy=500, aoa=config["aoa"])
    print(flow.units.convert_time_to_lu(30))
    print(flow.units.convert_time_to_lu(10))
    print(flow.units.convert_time_to_lu(20))
    RhoAverageReporter = AverageDensityField(interval=100,
                                            start_i_1=flow.units.convert_time_to_lu(20),
                                            start_i_2=flow.units.convert_time_to_lu(40),
                                            config = config)
    # collision = lt.BGKCollision(tau=flow.units.relaxation_parameter_lu)
    # collision = lt.KBCCollision(tau=flow.units.relaxation_parameter_lu)
    collision = lt.RegularizedCollision(tau=flow.units.relaxation_parameter_lu)
    simulation = lt.Simulation(flow=flow, collision=collision, reporter=[RhoAverageReporter])
    ...
    with torch.set_grad_enabled(config["train"]):
        ...
        # print(f"t_lu = {t_lu}")
        for i in range(int(flow.units.convert_time_to_lu(t_lu))):
            simulation(num_steps=1)
...
class NeuralTuning(torch.nn.Module):
    def __init__(self, dtype=torch.float64, device='cuda', nodes=20, index=None, K0Mul=1, K1Mul=5, K1Add=0, netversion=1):
        ...
        if netversion==1:
            print("Initialize a neural network architecutre 1.")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--Re", type=int, default=2000, help="")
    parser.add_argument("--aoa", type=int, default=10, help="")
    parser.add_argument("--gpd", type=int, default=500, help="")
    parser.add_argument("--Ma", type=float, default=0.1, help="")
    parser.add_argument("--xc", type=int, default=150)
    parser.add_argument("--t_lu", type=int, default=60) #866025
    ...
    parser.add_argument("--k_str", type=str, default="zou")
    parser.add_argument("--K1Mul", type=float, default=0)
    parser.add_argument("--K0Mul", type=float, default=0)
    ...
    K_tuned = NeuralTuning(K0Mul = args["K0Mul"],
                           ...
                           K1Add = args["K1Add"],
                           nodes = args["nodes"],
                           netversion=args["netversion"]) if args["K_neural"] else context.convert_to_tensor(torch.tensor([args["K0"], args["K1"]]).unsqueeze(0))
    ...
    # plt.imshow(flow.rho().cpu()[0].T); plt.colorbar(); plt.show()
    # plt.imshow(flow.u().cpu()[0].T); plt.colorbar(); plt.show()
    # plotRho(flow.f, flow=flow, config=args, slices=slices_plot, rectangle=rectangle)

    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 5))

    # --- Plot 1: Dichte (rho) ---
    # Wir nutzen ax1.imshow statt plt.imshow
    # im1 = ax1.imshow(flow.rho().cpu()[0].T, origin='lower', aspect='auto', vmin=0.9,vmax=1.1)
    im1 = ax1.imshow(reporter.postprocess().cpu()[0].T, origin='lower', aspect='auto')#, vmin=0.9, vmax=1.1)
    gpd = args["gpd"]
    aoa = args["aoa"]
    re = args["Re"]
    k0 = args["K0"]
    k1 = args["K1"]
    k_str = args["k_str"]
    torch.save(reporter.postprocess(),f"p_rms_nn_gpd{gpd}_re{re}_aoa{aoa}_k{k_str}_v3.pt")
    ax1.set_title("Dichte (rho)")
    fig.colorbar(im1, ax=ax1)

    # --- Plot 2: Geschwindigkeit (u - x-Komponente) ---
    # Wir nutzen ax2.imshow
    im2 = ax2.imshow(torch.norm(flow.u().cpu(),dim=0).T, origin='lower', aspect='auto', vmin=0,vmax=0.15)
    ax2.set_title("Geschwindigkeit u (x-Komponente)")
    fig.colorbar(im2, ax=ax2)

    # Layout optimieren, damit sich nichts überlappt
    plt.tight_layout()
    plt.show()
